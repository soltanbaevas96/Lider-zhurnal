...
export default function AttendancePicker({ groupId, lessonId, onChange }) {
      ...
      try {
        ...
        if (lessonId) {
          ...
          if (saved.length) {
            ...
            saved.forEach((r) => { init[r.student_id] = r.present })
          }
        }
  ...
  useEffect(() => {
    if (!students) return
    onChange(students.map((s) => ({ student_id: s.id, present: present[s.id] !== false })))
  }, [present, students])
  ...
  return (
    <div>
      ...
      <div style={{ border: `1px solid ${C.line}`, borderRadius: 11, overflow: 'hidden', maxHeight: 260, overflowY: 'auto' }}>
        {students.map((s, i) => {
          const here = present[s.id] !== false
          return (
            <button type="button" key={s.id} onClick={() => toggle(s.id)}
              style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderTop: i ? `1px solid ${C.line}` : 'none', background: here ? '#fff' : '#fdf6f2', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
